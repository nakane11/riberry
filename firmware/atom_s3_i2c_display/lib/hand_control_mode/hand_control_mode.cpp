#include <hand_control_mode.h>

HandControlMode* HandControlMode::instance = nullptr;

HandControlMode::HandControlMode(AtomS3LCD &lcd, AtomS3I2C &i2c)
  : atoms3lcd(lcd), atoms3i2c(i2c), Mode("HandControlMode") {
    instance = this;
}

void HandControlMode::task(void *parameter) {
  while (true) {
...
void HandControlMode::createTask(uint8_t xCoreID) {
  xTaskCreatePinnedToCore(task, "Hand Control Mode", 2048, NULL, 1, &taskHandle, xCoreID);
}
